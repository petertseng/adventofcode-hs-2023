...
astarInt :: forall a. (a -> Int) -> (a -> [(Int, a)]) -> (a -> Int) -> (a -> Bool) -> a -> Maybe Int
astarInt compress neigh heur goal start = astar' IntSet.empty (pqSingleton (heur start) (0, start))
  where astar' known tentative = pqPop tentative >>= (\((dist, cur), tent) -> expand known dist cur tent)
        expand :: IntSet -> Int -> a -> PQ (Int, a) -> Maybe Int
        expand _ dist current _ | goal current = Just dist
        expand known _ current tentative | compress current `IntSet.member` known = astar' known tentative
        expand known dist current tentative = astar' (IntSet.insert (compress current) known) tentative'
          where tentative' = pqUnion tentative (map estimateDist (filter unknown (neigh current)))
